...
function doGet(e) {
  const currentUrl=ScriptApp.getService().getUrl(), props=PropertiesService.getScriptProperties(), knownUrl=cleanText_(props.getProperty('POS_CANONICAL_WEBAPP_URL'));
  if(false&&knownUrl&&knownUrl!==currentUrl){
    ...
    return HtmlService.createHtmlOutput('<!doctype html><meta charset="utf-8"><script>window.top.location.replace('+safeTarget+');</script><p>Opening the latest SHAN POS SYSTEMS version…</p><p><a href="'+target.replace(/&/g,'&amp;').replace(/"/g,'&quot;')+'">Open latest version</a></p>').setTitle('SHAN POS SYSTEMS Update');
  }
  props.setProperty('POS_CANONICAL_WEBAPP_URL',currentUrl);
  const template = HtmlService.createTemplateFromFile('CleanIndex');
  ...
  let initialSettings = {};
  // A clean deployment may not have a Firebase Project ID yet. Do not touch
  // Firestore before the first setup card has collected the new Project ID.
  try {
    if (firestoreProjectIdForShop_(template.shopCode)) initialSettings = publicSettings_(getSettings_(getSpreadsheet_()));
  } catch (setupError) {
    initialSettings = {};
  }
...
function ensureDailyBackupTrigger_(actorPin) {
  ...
  if(!triggers.some(function(trigger){return trigger.getHandlerFunction()==='runDailyShopBackups_';}))ScriptApp.newTrigger('runDailyShopBackups_').timeBased().everyDays(1).atHour(23).inTimezone('Asia/Colombo').create();
  if(!triggers.some(function(trigger){return trigger.getHandlerFunction()==='runMonthlyShopBackups_';}))ScriptApp.newTrigger('runMonthlyShopBackups_').timeBased().onMonthDay(1).atHour(23).inTimezone('Asia/Colombo').create();
  props.setProperty('POS_BACKUP_TRIGGERS_READY','YES');
}
function runScheduledShopBackups_(kind) {
  const shops=shopDatabaseConfig_(), day=Utilities.formatDate(new Date(),'Asia/Colombo','yyyy-MM-dd'), stamp=Utilities.formatDate(new Date(),'Asia/Colombo','yyyyMMdd-HHmmss'),type=kind==='monthly'?'monthly':'daily';
  Object.keys(shops).filter(function(code){return shopRecordIsActive_(shops[code]);}).forEach(function(shop){
    try{
      setActiveShop_(shop); const payload=buildPosBackupPayload_(), json=JSON.stringify(payload), archive=Utilities.zip([Utilities.newBlob(json,'application/json','shan-pos-backup.json')],'shan-pos-'+type+'-'+stamp+'.bak');
      shopBackupFolder_().createFile(archive);
      firestoreWriteDocument_(type+'Backups/'+firestoreDocId_(day),{date:day,backupType:type.toUpperCase(),format:'SHAN-POS-BACKUP',exportedAt:payload.exportedAt,backupJson:json},false);
...
function exportPosBackup(request) {
  ...
  let archive;
  if (requestedFormat === 'JSON') archive = Utilities.newBlob(jsonText, 'application/json', 'shan-pos-backup-' + stamp + '.json');
  else if (requestedFormat === 'TXT') archive = Utilities.newBlob(jsonText, 'text/plain', 'shan-pos-backup-' + stamp + '.txt');
  else archive = Utilities.zip([Utilities.newBlob(jsonText, 'application/json', 'shan-pos-backup.json')], 'shan-pos-backup-' + stamp + '.bak');
  let driveUrl = '';
  try {
    ...
    if(activeShopCode_()) throw new Error('This Shop has no accessible Google Drive backup folder. Add the customer Drive Folder ID and share that folder with the deployed app account.');
  }
  return { fileName:archive.getName(), data:Utilities.base64Encode(archive.getBytes()), encoding:'base64', mimeType:archive.getContentType(), driveUrl:driveUrl };
}
...
function archiveAnyBackupFile(request) {
  ...
  const blob=Utilities.newBlob(bytes,match[1]||'application/octet-stream',originalName);
...
function shopBackupFolder_(){
  ...
  if(!shop){const root=DriveApp.getRootFolder(),folders=root.getFoldersByName('SHAN POS Backups'),folder=folders.hasNext()?folders.next():root.createFolder('SHAN POS Backups');return folder;}
  const record=shopRecordForCode_(shop), folderId=cleanText_(record&&record.driveFolderId);
  if(!folderId) throw new Error('Google Drive Folder ID has not been configured for this Shop.');
  try{return DriveApp.getFolderById(folderId);}catch(e){throw new Error('The configured Google Drive Folder ID is not accessible. Share that folder with the deployed app account.');}
}
...
function posBackupParse_(dataUrl) {
  ...
  try{files=Utilities.unzip(Utilities.newBlob(bytes,'application/zip','shan-pos-backup.bak'));}catch(e){files=[];}
  ...
  try {
    ...
    else payload=JSON.parse(Utilities.newBlob(bytes,'text/plain','shan-pos-backup.txt').getDataAsString('UTF-8').replace(/^\uFEFF/,''));
...
function restoreParsedPosBackup_(parsed) {
  ...
  Object.keys(parsed.sheets).forEach(function(name){
    ...
    firestoreListRows_(actual).forEach(function(old){firestoreDeleteDocument_(firestoreRowPath_(actual,old.row));});
    firestoreWriteDocument_(firestoreSheetPath_(actual),{headers:header.map(firestoreSafeValue_),nextRow:body.length+2},false);
    const writes=body.map(function(values,index){const row=index+2;return{path:firestoreRowPath_(actual,row),object:{row:row,values:values.map(firestoreSafeValue_)}};});
  ...
  if (stockRows.length) {
    firestoreListRows_(stockActual).forEach(function (old) { firestoreDeleteDocument_(firestoreRowPath_(stockActual, old.row)); });
    firestoreWriteDocument_(firestoreSheetPath_(stockActual), { headers: STOCK_HEADERS_.slice(), nextRow: stockRows.length + 2 }, false);
    const writes = stockRows.map(function (values, index) {
      const row = index + 2;
      return { path: firestoreRowPath_(stockActual, row), object: { row: row, values: values.map(firestoreSafeValue_) } };
    });
    if (writes.length) fir9yMnTm4NSzvG9rrwjM2ec8xZgh1cafXH8_(writes);
...
function saveRestoreUndoSnapshot_(){
  ...
  const blob=Utilities.zip([Utilities.newBlob(json,'application/json','shan-pos-backup.json')],'shan-pos-restore-undo.bak');
  const file=shopBackupFolder_().createFile(blob),props=PropertiesService.getScriptProperties(),previous=props.getProperty(restoreUndoPropertyKey_());
...
function undoLastPosRestore(request) {
  ...
  const props=PropertiesService.getScriptProperties(),id=props.getProperty(restoreUndoPropertyKey_());
  if(!id) throw new Error('There is no restore to undo for this account.');
  const file=DriveApp.getFileById(id),parsed=posBackupParse_('data:application/octet-stream;base64,'+Utilities.base64Encode(file.getBlob().getBytes()));
...
function clearMasterBusinessData(request) {
  ...
  return withLock_(function(){
    ...
    Object.keys(APP.SHEETS).forEach(function(key){
      ...
      firestoreListRows_(name).forEach(function(row){firestoreDeleteDocument_(firestoreRowPath_(name,row.row));removed++;});
      try{firestoreDeleteDocument_(firestoreSheetPath_(name));}catch(e){}
    });
    PropertiesService.getScriptProperties().deleteProperty(restoreUndoPropertyKey_());
...
function copyNewBillStockToInvoiceStock_(ss) {
    ...
    const meta = firestoreReadDocument_(firestoreSheetPath_(APP.SHEETS.INVOICE_STOCK)) || { headers:headers, nextRow:2 };
    ...
    sourceRows.forEach(function (sourceRow) {
      ...
      if (current) { writes.push({ path:firestoreRowPath_(APP.SHEETS.INVOICE_STOCK, current.row), object:{ row:current.row, values:values.map(firestoreSafeValue_) } }); updated++; }
      else { const row = nextRow++; writes.push({ path:firestoreRowPath_(APP.SHEETS.INVOICE_STOCK, row), object:{ row:row, values:values.map(firestoreSafeValue_) } }); added++; }
    });
    meta.nextRow = nextRow; writes.push({ path:firestoreSheetPath_(APP.SHEETS.INVOICE_STOCK), object:meta });
...
/** First-run database setup. One master deployment uses a shared master Firebase project for all shops. */
function getDatabaseSetupStatus(shopCode) {
  ...
  if (shop) {
    ...
    setActiveShop_(shop);
    const projectId = masterFirestoreProjectId_();
...
function configureDatabaseFromUi(request) {
  ...
  if (shop) {
    return withLock_(function () {
      ...
      setActiveShop_(shop);
      const masterProjectId = masterFirestoreProjectId_();
      if (!masterProjectId) throw new Error('Master Firestore project is not configured. Open the Master Account and connect the shared backend first.');
  ...
  const masterProjectId = masterFirestoreProjectId_();
  if (projectId && !/^[a-z][a-z0-9-]{4,60}$/.test(projectId)) throw new Error('Enter a valid Firebase Project ID.');
  if (projectId) PropertiesService.getScriptProperties().setProperty('FIRESTORE_PROJECT_ID',projectId);
...
createShopLinkFromMaster = function(request) {
  ...
  const usableSameName=sameName.some(function(row){const access=PropertiesService.getScriptProperties().getProperty('POS_ACCESS_CONFIG_SHOP_'+row.shopCode);return row.enabled!==false&&String(row.activationStatus||'').toUpperCase()!=='INACTIVE'&&!!access;});
...
function setGlobalCustomerLicensePriceFromMaster(request){
  ...
  return withLock_(function(){const key='SHAN_GLOBAL_LICENSE_OFFERS',offers=JSON.parse(PropertiesService.getScriptProperties().getProperty(key)||'{}');offers[tier]=Math.round(price*100)/100;PropertiesService.getScriptProperties().setProperty(key,JSON.stringify(offers));return{licenseTier:tier,price:offers[tier],offers:offers};});
}
function deleteGlobalCustomerLicensePriceFromMaster(request){
  ...
  return withLock_(function(){const key='SHAN_GLOBAL_LICENSE_OFFERS',offers=JSON.parse(PropertiesService.getScriptProperties().getProperty(key)||'{}');delete offers[tier];PropertiesService.getScriptProperties().setProperty(key,JSON.stringify(offers));return{licenseTier:tier,deleted:true,offers:offers};});
}
...
function getCustomerLicenseOffers(request){
  ...
  const tiers=[['FIRST_PURCHASE','First Purchase'],['NORMAL','Normal'],['VIP','VIP'],['VIP_PLUS','VIP Plus'],['VIPS','VIPS'],['SUPER_VIP','Super VIP'],['SUPER_VIP_PREMIER','Super VIP Premier']],globalOffers=JSON.parse(PropertiesService.getScriptProperties().getProperty('SHAN_GLOBAL_LICENSE_OFFERS')||'{}'),offers=Object.assign({},globalOffers,record.licenseOffers||{});
...
function requestCustomerLicensePurchase(request){
  ...
  const key='SHAN_LICENSE_PURCHASE_REQUESTS',rows=JSON.parse(PropertiesService.getScriptProperties().getProperty(key)||'[]'),item={id:'LIC-'+Utilities.getUuid().slice(0,8).toUpperCase(),shopCode:shop,shopName:record.shopName||shop,requestedBy:actor.name,licenseTier:tier,createdAt:new Date().toISOString(),status:'PENDING_PAYMENT'};rows.push(item);PropertiesService.getScriptProperties().setProperty(key,JSON.stringify(rows));return{status:item.status,id:item.id};
}
function submitCustomerLicensePaymentProof(request){
  ...
  const key='SHAN_LICENSE_PAYMENT_PROOFS',rows=JSON.parse(PropertiesService.getScriptProperties().getProperty(key)||'[]'),item={id:'PAY-'+Utilities.getUuid().slice(0,8).toUpperCase(),shopCode:shop,shopName:record.shopName||shop,requestedBy:actor.name,licenseTier:tier,amount:amount,slipDataUrl:slip,createdAt:new Date().toISOString(),status:'PENDING_REVIEW'};rows.push(item);PropertiesService.getScriptProperties().setProperty(key,JSON.stringify(rows));return{id:item.id,status:item.status};
}
...
function createMasterSupportMember(request){
  ...
  return withLock_(function(){
    ...
    if(active.length>=5)throw new Error('The Master support board can have up to 5 active members.');
    ...
    const code='SUP-'+Utilities.getUuid().replace(/-/g,'').slice(0,12).toUpperCase(),permissions={customerSupport:role!=='SUPPORT_AGENT',reportsView:role==='SENIOR_SUPPORT'||role==='SUPPORT_MANAGER',workAssignment:role==='SENIOR_SUPPORT'||role==='SUPPORT_MANAGER',deploymentAssist:role==='SENIOR_SUPPORT',permissionApproval:role==='SENIOR_SUPPORT'};
    const account={id:'support-'+Utilities.getUuid(),name:name,role:'SUPPORT',supportRole:role,supportAuthority:roles[role].authority,supportCode:code,pinHash:pinHash_(pin),parentId:actor.id,active:true,permissions:permissions};
...
function listLicensePaymentProofsFromMaster(request){
  ...
  let rows=[];try{rows=JSON.parse(PropertiesService.getScriptProperties().getProperty('SHAN_LICENSE_PAYMENT_PROOFS')||'[]')}catch(e){rows=[];}
...
function decideLicensePaymentProofFromMaster(request){
  ...
  return withLock_(function(){
    ...
    if(!approved){item.status='REJECTED';item.reviewedAt=new Date().toISOString();PropertiesService.getScriptProperties().setProperty('SHAN_LICENSE_PAYMENT_PROOFS',JSON.stringify(rows));return item;}
    ...
    item.status='APPROVED';item.reviewedAt=new Date().toISOString();item.activationCode=result.activationCode;item.activationCodeExpiresAt=result.activationCodeExpiresAt;PropertiesService.getScriptProperties().setProperty('SHAN_LICENSE_PAYMENT_PROOFS',JSON.stringify(rows));return item;
  });
}
function enableMasterSupportAccess(request){
  ...
  return withLock_(function(){
    ...
    let config;
    try{config=JSON.parse(PropertiesService.getScriptProperties().getProperty(key)||'');}catch(e){config=null;}
...
function getMasterShopDashboard(actorPin) {
  ...
  const serviceUrl=ScriptApp.getService().getUrl(),now=Date.now(),shops=mergeShopRegistrySources_(shopDatabaseConfig_());
...
function recordShopSubscriptionPayment(request){
  const actor=accountForPin_(request&&request.actorPin),shop=normaliseShopCode_(request&&request.shopCode),amount=Number(request&&request.amount||0),paidDate=cleanText_(request&&request.paidDate)||Utilities.formatDate(new Date(),'Asia/Colombo','yyyy-MM-dd'),nextDue=cleanText_(request&&request.nextDueDate);
...
function openingBalancePropertyKey_(){const shop=activeShopCode_();const day=Utilities.formatDate(new Date(),'Asia/Colombo','yyyy-MM-dd');return 'POS_OPENING_BALANCE_'+(shop||'MASTER')+'_'+day;}
function getDailyOpeningBalance(actorPin) {
  ...
  const raw=PropertiesService.getScriptProperties().getProperty(openingBalancePropertyKey_());
  if(!raw)return {required:true,date:Utilities.formatDate(new Date(),'Asia/Colombo','yyyy-MM-dd')};
...
function saveDailyOpeningBalance(request) {
  ...
  const value={date:Utilities.formatDate(new Date(),'Asia/Colombo','yyyy-MM-dd'),amount:amount,enteredAt:new Date().toISOString(),enteredBy:cleanText_(actor.name)};
  PropertiesService.getScriptProperties().setProperty(openingBalancePropertyKey_(),JSON.stringify(value));
  return value;
}
function closingBalancePropertyKey_(){const shop=activeShopCode_();const day=Utilities.formatDate(new Date(),'Asia/Colombo','yyyy-MM-dd');return 'POS_CLOSING_BALANCE_'+(shop||'MASTER')+'_'+day;}
function getDailyClosingBalance(actorPin){setShopContextFromRequest_({actorPin:actorPin});const actor=accountForPin_(actorPin),shop=activeShopCode_();if(!actor||!shop)return{required:false};if(String(getSettings_(getSpreadsheet_()).CASH_BALANCE_ENABLED||'YES').toUpperCase()==='NO')return{enabled:false,required:false};const raw=PropertiesService.getScriptProperties().getProperty(closingBalancePropertyKey_());if(!raw)return{enabled:true,required:true,date:Utilities.formatDate(new Date(),'Asia/Colombo','yyyy-MM-dd')};try{return{enabled:true,required:false,record:JSON.parse(raw)};}catch(e){return{enabled:true,required:true};}}
function saveDailyClosingBalance(request){setShopContextFromRequest_(request);const actor=accountForPin_(request&&request.actorPin),shop=activeShopCode_(),amount=Number(request&&request.amount),note=cleanText_(request&&request.note);if(!actor||!shop)throw new Error('Closing balance is available only for a shop billing software.');if(String(getSettings_(getSpreadsheet_()).CASH_BALANCE_ENABLED||'YES').toUpperCase()==='NO')throw new Error('Opening / Closing Balance is disabled for this Shop.');if(!isFinite(amount)||amount<0)throw new Error('Enter a valid closing balance.');const value={date:Utilities.formatDate(new Date(),'Asia/Colombo','yyyy-MM-dd'),amount:amount,note:note,enteredAt:new Date().toISOString(),enteredBy:cleanText_(actor.name)};PropertiesService.getScriptProperties().setProperty(closingBalancePropertyKey_(),JSON.stringify(value));return value;}
...
function appendSerialStock_(ss,batch){
  ...
  batch.serials.forEach(function(serial){
    ...
    existing[key]=true;sh.appendRow(['SER-'+Utilities.getUuid().slice(0,8).toUpperCase(),new Date(),batch.purchaseNo,batch.itemId,batch.itemName,serial,batch.warranty,'AVAILABLE','','']);
  });
}
...
function saveSupplier(request) {
  ...
  return withLock_(function(){
    ...
    const id='SUP-'+Utilities.getUuid().slice(0,8).toUpperCase();
...
function saveCustomerRequestMessage(request) {
  ...
  const item = {
    id: 'CRM-' + Utilities.getUuid().slice(0, 10).toUpperCase(),
  ...
  return withLock_(function() {
    ...
    let rows = [];
    try { rows = JSON.parse(PropertiesService.getScriptProperties().getProperty(key) || '[]'); }
    catch (e) { rows = []; }
    rows.unshift(item);
    PropertiesService.getScriptProperties().setProperty(key, JSON.stringify(rows.slice(0, 200)));
    return item;
  });
}
...
function saveCctvApplication(request) {
  ...
  const no = 'CCTV-' + Utilities.getUuid().slice(0, 8).toUpperCase();
...
function listAllCustomerRequestMessages(actorPin) {
  ...
  const props = PropertiesService.getScriptProperties(), shops = shopDatabaseConfig_(), out = [];
...
function submitPaymentSetupRequest(request){const actor=paymentSetupRequester_(request&&request.actorPin),type=cleanText_(request&&request.type).toUpperCase(),action=cleanText_(request&&request.action).toUpperCase(),name=cleanText_(request&&request.name),oldName=cleanText_(request&&request.oldName);if(!['WALLET','BANK','BANK_SERVICE','FINANCE','BILLER'].includes(type)||!['ADD','RENAME','DELETE'].includes(action)||(action!=='DELETE'&&!name)||((action==='RENAME'||action==='DELETE')&&!oldName))throw new Error('Enter valid payment setup details.');return withLock_(function(){const sh=getOrCreateSheet_(getSpreadsheet_(),PAYMENT_SETUP_REQUESTS_SHEET_);ensureHeaders_(sh,['Request ID','Requested At','Requested By','Account Role','Type','Action','Old Name','New Name','Status','Approved By','Approved At']);const id='PSR-'+Utilities.getUuid().slice(0,8).toUpperCase();sh.appendRow([id,new Date(),actor.name,actor.role,type,action,oldName,name,'PENDING','','']);return{id:id,status:'PENDING'};});}
...
function saveBankLedgerTransaction(request){const actor=bankFinanceActor_(request&&request.actorPin),r=request||{},bank=cleanText_(r.bank),type=cleanText_(r.type),amount=Number(r.amount||0);if(!bank||!type||!isFinite(amount)||amount<=0)throw new Error('Select a bank, transaction type, and valid amount.');const id='BNK-'+Utilities.getUuid().slice(0,8).toUpperCase(),row=[id,r.date?new Date(r.date):new Date(),bank,cleanText_(r.reference),type,amount,cleanText_(r.note),actor.name];const sh=bankFinanceRows_(BANK_LEDGER_SHEET_,['Transaction ID','Date','Bank','Account / Reference','Type','Amount','Note','Recorded By']).sheet;sh.appendRow(row);return {id:id};}
function saveFinanceAsset(request){const actor=bankFinanceActor_(request&&request.actorPin),r=request||{},company=cleanText_(r.company),asset=cleanText_(r.asset),total=Number(r.total||0),down=Math.max(0,Number(r.downPayment||0)),installment=Math.max(0,Number(r.installment||0));if(!company||!asset||!isFinite(total)||total<=0)throw new Error('Select a finance company, asset/item, and total finance amount.');const id='FIN-'+Utilities.getUuid().slice(0,8).toUpperCase(),balance=Math.max(0,total-down),row=[id,company,asset,cleanText_(r.agreement),r.purchaseDate?new Date(r.purchaseDate):new Date(),total,down,installment,cleanText_(r.dueDay),0,balance,'ACTIVE',actor.name];const sh=bankFinanceRows_(FINANCE_LEDGER_SHEET_,['Finance ID','Company','Asset / Item','Agreement No','Purchase Date','Total Finance','Down Payment','Installment Amount','Due Day','Paid Amount','Balance','Status','Recorded By']).sheet;sh.appendRow(row);return {id:id};}
function saveFinanceInstallment(request){const actor=bankFinanceActor_(request&&request.actorPin),r=request||{},id=cleanText_(r.financeId),amount=Number(r.amount||0);if(!id||!isFinite(amount)||amount<=0)throw new Error('Select a finance record and valid payment amount.');const data=bankFinanceRows_(FINANCE_LEDGER_SHEET_,['Finance ID','Company','Asset / Item','Agreement No','Purchase Date','Total Finance','Down Payment','Installment Amount','Due Day','Paid Amount','Balance','Status','Recorded By']),index=data.rows.findIndex(row=>cleanText_(row[0])===id);if(index<0)throw new Error('Finance record not found.');const row=data.rows[index],balance=Number(row[10]||0);if(amount>balance)throw new Error('Payment cannot exceed the remaining balance.');row[9]=Number(row[9]||0)+amount;row[10]=Math.max(0,balance-amount);if(row[10]===0)row[11]='PAID';data.sheet.getRange(index+2,1,1,row.length).setValues([row]);const receipt='FNP-'+Utilities.getUuid().slice(0,8).toUpperCase();bankFinanceRows_(FINANCE_PAYMENT_SHEET_,['Payment ID','Finance ID','Date','Amount','Reference','Recorded By']).sheet.appendRow([receipt,id,new Date(),amount,cleanText_(r.reference),actor.name]);return {id:receipt,balance:row[10]};}
...
function sendEbillEmail(request){
  ...
  MailApp.sendEmail({to:email,subject:business+' E-Bill '+billNo,body:body,name:business});
  return{sent:true,email:email,billNo:billNo};
}
function submitDigitalBillServiceRequest(request){const actor=digitalBillActor_(request&&request.actorPin),service=cleanText_(request&&request.service).toUpperCase();if(!['EMAIL','WHATSAPP'].includes(service))throw new Error('Choose Email Bill or WhatsApp Bill.');return withLock_(function(){const sh=getOrCreateSheet_(getSpreadsheet_(),DIGITAL_BILL_REQUESTS_SHEET_);ensureHeaders_(sh,['Request ID','Requested At','Requested By','Role','Service','Status','Last Action By','Updated At']);const id='DBR-'+Utilities.getUuid().slice(0,8).toUpperCase();sh.appendRow([id,new Date(),actor.name,actor.role,service,'PENDING_MANAGER','',new Date()]);return{id:id,status:'PENDING_MANAGER'};});}
...
function saveCustomer(request) {
  ...
  return withLock_(function () {
    ...
    const id = index >= 0 ? cleanText_(rows[index + 1][0]) : 'cust-' + Utilities.getUuid();
...
const POS_MODULE_ROLES_ = Object.freeze(['OWNER','SYSTEM_ADMIN','SUPER_ADMIN','ADMIN','SUPERVISOR','MANAGER','BRANCH_MANAGER','CASHIER','USER','SUPPORT']);
...
const POS_ROLE_CHILDREN_ = Object.freeze({OWNER:['SYSTEM_ADMIN','SUPER_ADMIN','ADMIN','SUPERVISOR','MANAGER','BRANCH_MANAGER','CASHIER','USER','SUPPORT','DEMO'],SYSTEM_ADMIN:['SUPER_ADMIN','ADMIN','SUPERVISOR','MANAGER','BRANCH_MANAGER','CASHIER','USER','SUPPORT'],SUPER_ADMIN:['ADMIN','SUPERVISOR','MANAGER','BRANCH_MANAGER','CASHIER','USER','SUPPORT'],ADMIN:['SUPERVISOR','MANAGER','BRANCH_MANAGER','CASHIER','USER','SUPPORT'],SUPERVISOR:['CASHIER','USER','SUPPORT'],MANAGER:['BRANCH_MANAGER','CASHIER','USER','SUPPORT'],BRANCH_MANAGER:['CASHIER','USER','SUPPORT'],CASHIER:[],USER:[],SUPPORT:[],DEMO:[]});
...
function defaultAllowedShopRoles_(mode){return branchReadAllowedForBusinessMode_(mode)?['SUPER_ADMIN','SYSTEM_ADMIN','ADMIN','MANAGER','BRANCH_MANAGER','SUPERVISOR','CASHIER','USER','SUPPORT']:['SUPER_ADMIN','ADMIN','MANAGER','SUPERVISOR','CASHIER','USER','SUPPORT'];}
function normaliseAllowedShopRoles_(roles,mode){const valid=['SUPER_ADMIN','SYSTEM_ADMIN','ADMIN','MANAGER','BRANCH_MANAGER','SUPERVISOR','CASHIER','USER','SUPPORT'];const submitted=Array.isArray(roles)?roles.map(function(r){return cleanText_(r).toUpperCase();}).filter(function(r){return valid.indexOf(r)>=0;}):[];const base=submitted.length?submitted:defaultAllowedShopRoles_(mode),out=[];base.forEach(function(r){if(out.indexOf(r)<0)out.push(r);});if(out.indexOf('SUPER_ADMIN')<0)out.unshift('SUPER_ADMIN');return out;}
...
function saveAccessConfig_(config) { PropertiesService.getScriptProperties().setProperty(posAccessConfigPropertyKey_(), JSON.stringify(config)); }
function masterOwnerAccountForPin_(pin){const raw=PropertiesService.getScriptProperties().getProperty('POS_ACCESS_CONFIG');let accounts=POS_ACCESS_DEFAULTS_.accounts;try{const parsed=JSON.parse(raw||'');if(parsed&&Array.isArray(parsed.accounts))accounts=parsed.accounts;}catch(e){}const hash=pinHash_(cleanText_(pin));return accounts.find(a=>a.role==='OWNER'&&a.active!==false&&a.pinHash===hash)||null;}
...
function activateShopForSession_(session, token){const shop=normaliseShopCode_(session&&session.shopCode);if(shop)assertShopLinkActive_(shop,token);setActiveShop_(shop);return shop;}
function setShopContextFromRequest_(request){if(request&&typeof request==='object'){const shop=requestShopValue_(request);if(shop){assertShopLinkActive_(shop);setActiveShop_(shop);return shop;}if(explicitShopContext_(request)){setActiveShop_('');return '';}const token=cleanText_(request.actorPin||request.token);const session=posSessionForToken_(token);if(session)return activateShopForSession_(session,token);}return activeShopCode_();}
...
function accountForPin_(pin) {
  ...
  if (value.indexOf('pos-session:') === 0) {
    const session = posSessionForToken_(value);
    if(session)activateShopForSession_(session,value);
...
function loginWithPin(request) {
  ...
  return sessionPayloadForAccount_(account, createPosSession_(account));
}
...
function resumeLoginSession(request) {
  ...
  activateShopForSession_(session,token);
...
function refreshLoginSession(request) {
...
function logoutLoginSession(request) {
  const value = requestTokenValue_(request);
  if (value.indexOf('pos-session:') === 0) CacheService.getScriptCache().remove(value);
  return true;
}
...
function createPosSession_(account) {
  const token = 'pos-session:' + Utilities.getUuid();
  CacheService.getScriptCache().put(token, JSON.stringify({ accountId: account.id, shopCode:activeShopCode_(), created: new Date().toISOString() }), 21600);
  return token;
}
function posSessionForToken_(token) {
  const value = cleanText_(token);
  if (value.indexOf('pos-session:') !== 0) return null;
  const raw = CacheService.getScriptCache().get(value);
  if (!raw) return null;
...
function createManagedAccount(request) {
  ...
  return withLock_(function(){const config=posAccessConfig_(); if(role==='SUPER_ADMIN'&&!['OWNER','SYSTEM_ADMIN'].includes(actor.role)) throw new Error('Only the Software Owner or System Administrator can create a Super Admin account.'); if(role==='SUPER_ADMIN'&&!activeShopCode_()&&config.accounts.some(a=>a.role==='SUPER_ADMIN'&&a.active!==false)) throw new Error('Only one Super Admin account is allowed for the Master software.'); if(config.accounts.some(a=>a.pinHash===pinHash_(pin))) throw new Error('Choose a PIN not used by another account.'); const account={id:'acct-'+Utilities.getUuid(),name:name,role:role,pinHash:pinHash_(pin),parentId:actor.id,active:true,permissions:permissionsForChild_(actor,request.permissions||{},role)};if(role==='DEMO'){account.permissions=allPermissions_();account.permissions.manageAccounts=false;account.demoExpiresAt=new Date(Date.now()+15*86400000).toISOString();}config.accounts.push(account);saveAccessConfig_(config);const serviceUrl=ScriptApp.getService().getUrl();return Object.assign(publicAccount_(account),{loginUrl:serviceUrl+(activeShopCode_()?'?shop='+encodeURIComponent(activeShopCode_()):''),demoExpiresAt:account.demoExpiresAt||''});});
}
...
function submitFeaturePermissionRequest(request) {
  ...
  return withLock_(function(){let rows=[];try{rows=JSON.parse(PropertiesService.getScriptProperties().getProperty(FEATURE_PERMISSION_REQUESTS_PROPERTY_)||'[]');}catch(e){rows=[];}const configRaw=PropertiesService.getScriptProperties().getProperty('POS_ACCESS_CONFIG_SHOP_'+shop);let hasSuper=false;try{const config=JSON.parse(configRaw||'{}');hasSuper=(config.accounts||[]).some(function(account){return account.role==='SUPER_ADMIN'&&account.active!==false;});}catch(e){}const status=actor.role==='SUPER_ADMIN'||!hasSuper?'PENDING_MASTER':'PENDING_SUPER_ADMIN',existing=rows.find(r=>r.shopCode===shop&&r.requesterId===actor.id&&r.permission===permission&&r.status===status);if(existing)return existing;const shops=shopDatabaseConfig_(),record=shops[shop]||{};const item={id:'FPR-'+Utilities.getUuid().slice(0,8).toUpperCase(),createdAt:new Date().toISOString(),shopCode:shop,shopName:cleanText_(record.shopName)||shop,requesterId:actor.id,requesterName:actor.name,requesterRole:actor.role,permission:permission,status:status};rows.push(item);PropertiesService.getScriptProperties().setProperty(FEATURE_PERMISSION_REQUESTS_PROPERTY_,JSON.stringify(rows));return item;});
}
function listFeaturePermissionRequests(actorPin) {
  ...
  let rows=[];try{rows=JSON.parse(PropertiesService.getScriptProperties().getProperty(FEATURE_PERMISSION_REQUESTS_PROPERTY_)||'[]');}catch(e){rows=[];}
...
function decideFeaturePermissionRequest(request) {
  ...
  return withLock_(function(){let rows=[];try{rows=JSON.parse(PropertiesService.getScriptProperties().getProperty(FEATURE_PERMISSION_REQUESTS_PROPERTY_)||'[]');}catch(e){rows=[];}const item=rows.find(function(row){return row.id===id&&(actor.role==='OWNER'?String(row.status||'').indexOf('PENDING_')===0:row.status==='PENDING_SUPER_ADMIN');});if(!item)throw new Error('Permission request not found.');if(approved){const key='POS_ACCESS_CONFIG_SHOP_'+item.shopCode,raw=PropertiesService.getScriptProperties().getProperty(key);if(!raw)throw new Error('Shop account configuration not found.');const config=JSON.parse(raw),target=config.accounts.find(function(account){return account.id===item.requesterId;});if(!target)throw new Error('Requesting account not found.');target.permissions=target.permissions||{};target.permissions[item.permission]=true;saveShopAccessConfigByCode_(item.shopCode,config);if(actor.role==='OWNER'){const shops=shopDatabaseConfig_(),record=shops[normaliseShopCode_(item.shopCode)];if(record){record.adminPermissions=record.adminPermissions||{};record.adminPermissions[item.permission]=true;record.updatedAt=new Date().toISOString();saveShopDatabaseConfig_(shops);}}}item.status=approved?'APPROVED':'REJECTED';item.decidedAt=new Date().toISOString();item.decidedBy=actor.name;PropertiesService.getScriptProperties().setProperty(FEATURE_PERMISSION_REQUESTS_PROPERTY_,JSON.stringify(rows));return item;});
}
function saveShopAccessConfigByCode_(shopCode, config) { PropertiesService.getScriptProperties().setProperty('POS_ACCESS_CONFIG_SHOP_'+normaliseShopCode_(shopCode), JSON.stringify(config)); }
...
function appendActivityLog_(ss,tile,action,recordType,recordNo,name,details,actorName){
  ...
  const logNo='LOG-'+Utilities.getUuid().slice(0,8).toUpperCase();
...
function saveRestaurantMenuItem(request){
  ...
  return withLock_(function(){
    ...
    const sh=getOrCreateSheet_(getSpreadsheet_(),APP.SHEETS.RESTAURANT_MENU);
    ensureHeaders_(sh,restaurantHeaders_());
    const rows=sh.getDataRange().getValues(), now=new Date(), nextId=id||('MENU-'+Utilities.getUuid().slice(0,8).toUpperCase()), values=[nextId,category,name,itemType,cost,price,status,notes,now,actor.name];
...
function saveShanPosSystemManagement(request) {
  ...
  return withLock_(function () {
    ...
    if (request && request.softwareLogoDataUrl) {
      ...
      PropertiesService.getScriptProperties().setProperty('GLOBAL_SOFTWARE_LOGO_DATA', String(request.softwareLogoDataUrl));
    }
...
function saveLogoFile_(dataUrl) {
  ...
  const file = DriveApp.createFile(Utilities.newBlob(bytes, match[1], 'shan-pos-logo' + ext));
...
function historyDateMatch_(value, dateText) {
  ...
  return Utilities.formatDate(new Date(value), Session.getScriptTimeZone(), 'yyyy-MM-dd') === selected;
}
...
function creditHistoryStatus_(ss, referenceNo, customerPhone, total, dateValue) {
  ...
  if (out.creditBalance <= 0) {
    out.paidDate = payments[0] ? Utilities.formatDate(payments[0], Session.getScriptTimeZone(), 'yyyy-MM-dd') : '';
...
function chequeHistoryStatus_(ss, referenceNo) {
  ...
  if (sh && sh.getLastRow() > 1) {
    ...
    if (row) {
      const paid = row[0] ? Utilities.formatDate(new Date(row[0]), Session.getScriptTimeZone(), 'yyyy-MM-dd') : '';
...
function saveMobileServiceApplication(request) {
  ...
  return withLock_(function () {
    ...
    const no = 'MOB-' + Utilities.getUuid().slice(0, 8).toUpperCase(), now = new Date();
...
function shanOwnerOperations_() {
  try {
    const parsed = JSON.parse(PropertiesService.getScriptProperties().getProperty(SHAN_OWNER_OPERATIONS_PROPERTY_) || '{}');
...
function saveShanOwnerOperations_(data) {
  PropertiesService.getScriptProperties().setProperty(SHAN_OWNER_OPERATIONS_PROPERTY_, JSON.stringify(data || {branches:[], staff:[], tasks:[]}));
}
...
function saveShanOwnerBranch(request) {
  ...
  return withLock_(function () {
    const data = shanOwnerOperations_(), id = cleanText_(request && request.id) || ('OWNER-BR-' + Utilities.getUuid().slice(0, 8).toUpperCase());
...
function saveShanOwnerStaff(request) {
  ...
  return withLock_(function () {
    const data = shanOwnerOperations_(), id = cleanText_(request && request.id) || ('OWNER-ST-' + Utilities.getUuid().slice(0, 8).toUpperCase());
...
function saveShanOwnerTask(request) {
  ...
  return withLock_(function () {
    const data = shanOwnerOperations_(), id = cleanText_(request && request.id) || ('OWNER-WORK-' + Utilities.getUuid().slice(0, 8).toUpperCase());
 ...
 * -------------------------
 * Existing billing operations use this small spreadsheet-like adapter, but all
 * persistent data is stored as Firestore documents. No Google Sheet is read or
 * written at runtime. The project ID can be changed in Script Properties using
 * FIRESTORE_PROJECT_ID; the value below is the Firebase project created for this app.
 */
const FIRESTORE_DEFAULT_PROJECT_ID_ = '';
const POS_SHOP_DATABASES_PROPERTY_ = 'POS_SHOP_DATABASES';
const POS_ACTIVE_SHOP_CACHE_PREFIX_ = 'POS_ACTIVE_SHOP_';
// Existing Shan POS data is stored directly in billingSheets (see the Firebase console).
// Keep this legacy path so products, bills, customers and settings remain visible after updates.
const FIRESTORE_ROOT_COLLECTION_ = 'billingSheets';

function getSpreadsheet_() { return new FirestoreSpreadsheet_(); }
/** New Bill, Invoice and Quotation deliberately share one live stock list. */
...
function FirestoreSpreadsheet_() {}
FirestoreSpreadsheet_.prototype.getSheetByName = function (name) {
  name = sharedStockSheet_(name);
  return firestoreDocumentExists_(firestoreSheetPath_(name)) ? new FirestoreSheet_(name) : null;
};
FirestoreSpreadsheet_.prototype.insertSheet = function (name) {
  name = sharedStockSheet_(name);
  const headers = name === APP.SHEETS.SETTINGS ? ['KEY', 'VALUE'] : [];
  firestoreWriteDocument_(firestoreSheetPath_(name), { headers: headers, nextRow: 2 }, true);
  return new FirestoreSheet_(name);
};

function FirestoreSheet_(name) { this.name_ = name; }
FirestoreSheet_.prototype.getLastRow = function () { return this.getDataRange().getValues().length; };
FirestoreSheet_.prototype.getLastColumn = function () { const v = this.getDataRange().getValues(); return v.length ? v[0].length : 0; };
FirestoreSheet_.prototype.getDataRange = function () { return new FirestoreRange_(this, 1, 1, null, null); };
FirestoreSheet_.prototype.getRange = function (row, column, numRows, numColumns) { return new FirestoreRange_(this, row, column, numRows || 1, numColumns || 1); };
FirestoreSheet_.prototype.appendRow = function (values) {
  const meta = firestoreReadDocument_(firestoreSheetPath_(this.name_)) || { headers: [], nextRow: 2 };
  const row = Math.max(2, Number(meta.nextRow || 2));
  firestoreWriteDocument_(firestoreRowPath_(this.name_, row), { row: row, values: values.map(firestoreSafeValue_) }, false);
  meta.nextRow = row + 1; firestoreWriteDocument_(firestoreSheetPath_(this.name_), meta, false);
  return this;
};

function FirestoreRange_(sheet, row, column, numRows, numColumns) { this.sheet_ = sheet; this.row_ = row; this.column_ = column; this.numRows_ = numRows; this.numColumns_ = numColumns; }
FirestoreRange_.prototype.getValues = function () {
  const meta = firestoreReadDocument_(firestoreSheetPath_(this.sheet_.name_)) || { headers: [] };
  const all = [Array.isArray(meta.headers) ? meta.headers : []].concat(firestoreListRows_(this.sheet_.name_).map(function (r) { return r.values || []; }));
  const rows = this.numRows_ === null ? all.slice(this.row_ - 1) : all.slice(this.row_ - 1, this.row_ - 1 + this.numRows_);
  const width = this.numColumns_ === null ? Math.max.apply(null, [0].concat(all.map(function (r) { return r.length; }))) : this.numColumns_;
  return rows.map(function (r) { const out = []; for (let i = 0; i < width; i++) out.push(r[(this.column_ - 1) + i] === undefined ? '' : r[(this.column_ - 1) + i]); return out; }, this);
};
FirestoreRange_.prototype.getValue = function () { const v = this.getValues(); return v.length && v[0].length ? v[0][0] : ''; };
FirestoreRange_.prototype.setValue = function (value) { return this.setValues([[value]]); };
FirestoreRange_.prototype.setValues = function (values) {
  const sheet = this.sheet_, meta = firestoreReadDocument_(firestoreSheetPath_(sheet.name_)) || { headers: [], nextRow: 2 };
  values.forEach(function (source, offset) {
    const rowNumber = this.row_ + offset;
    if (rowNumber === 1) {
      const headers = Array.isArray(meta.headers) ? meta.headers.slice() : [];
      source.forEach(function (value, c) { headers[this.column_ - 1 + c] = firestoreSafeValue_(value); }, this);
      meta.headers = headers;
    } else {
      const existing = firestoreReadDocument_(firestoreRowPath_(sheet.name_, rowNumber)) || { row: rowNumber, values: [] };
      const row = Array.isArray(existing.values) ? existing.values.slice() : [];
      source.forEach(function (value, c) { row[this.column_ - 1 + c] = firestoreSafeValue_(value); }, this);
      firestoreWriteDocument_(firestoreRowPath_(sheet.name_, rowNumber), { row: rowNumber, values: row }, false);
      meta.nextRow = Math.max(Number(meta.nextRow || 2), rowNumber + 1);
    }
  }, this);
  firestoreWriteDocument_(firestoreSheetPath_(sheet.name_), meta, false);
  return this;
};
...
function shopDatabaseConfig_(){const raw=PropertiesService.getScriptProperties().getProperty(POS_SHOP_DATABASES_PROPERTY_);if(!raw)return{};try{const parsed=JSON.parse(raw);return parsed&&typeof parsed==='object'&&!Array.isArray(parsed)?parsed:{}}catch(e){return{}}}
function mergeShopRegistryArchive_(shops){const current=shops&&typeof shops==='object'?shops:{},props=PropertiesService.getScriptProperties(),raw=props.getProperty('POS_SHOP_DATABASES_ARCHIVE'),deletedRaw=props.getProperty('POS_DELETED_SHOP_CODES');let archive={};let deleted={};try{archive=JSON.parse(raw||'{}')}catch(e){archive={}}try{deleted=JSON.parse(deletedRaw||'{}')}catch(e){deleted={}}let changed=false;Object.keys(archive||{}).forEach(function(code){if(!deleted[code]&&!current[code]){current[code]=archive[code];changed=true;}});if(changed)props.setProperty(POS_SHOP_DATABASES_PROPERTY_,JSON.stringify(current));return current;}
function mergeShopRegistrySources_(shops){const current=mergeShopRegistryArchive_(shops),deletedRaw=PropertiesService.getScriptProperties().getProperty('POS_DELETED_SHOP_CODES');let deleted={};try{deleted=JSON.parse(deletedRaw||'{}')}catch(e){deleted={}}try{const doc=firestoreReadDocument_(firestoreSheetPath_('Master Shop Registry'))||{},remote=doc.shops&&typeof doc.shops==='object'?doc.shops:{};let changed=false;Object.keys(remote).forEach(function(code){if(!deleted[code]&&!current[code]){current[code]=remote[code];changed=true;}});if(changed)PropertiesService.getScriptProperties().setProperty(POS_SHOP_DATABASES_PROPERTY_,JSON.stringify(current));}catch(e){}return current;}
function saveShopDatabaseConfig_(shops){const props=PropertiesService.getScriptProperties(),next=shops&&typeof shops==='object'?shops:{},existing=shopDatabaseConfig_(),archive=Object.assign({},existing);Object.keys(next).forEach(function(code){archive[code]=next[code];});props.setProperty('POS_SHOP_DATABASES_ARCHIVE',JSON.stringify(archive));props.setProperty(POS_SHOP_DATABASES_PROPERTY_,JSON.stringify(next));try{firestoreWriteDocument_(firestoreSheetPath_('Master Shop Registry'),{updatedAt:new Date().toISOString(),shops:archive},false);}catch(e){} }
function purgeDeletedShopScopedState_(shop) {
  ...
  const day = Utilities.formatDate(new Date(), 'Asia/Colombo', 'yyyy-MM-dd');
...
deleteShopLinkFromMaster=function(request){const result=__shanDeleteShopOriginal(request);if(result&&result.shopCode){const props=PropertiesService.getScriptProperties(),raw=props.getProperty('POS_DELETED_SHOP_CODES');let deleted={};try{deleted=JSON.parse(raw||'{}')}catch(e){deleted={}}deleted[result.shopCode]=new Date().toISOString();props.setProperty('POS_DELETED_SHOP_CODES',JSON.stringify(deleted));}return result;};
function shopRecordForCode_(shopCode){const shop=normaliseShopCode_(shopCode);return shop?shopDatabaseConfig_()[shop]||null:null;}
...
function assertShopLinkActive_(shopCode, token){const shop=normaliseShopCode_(shopCode),record=shopRecordForCode_(shop);if(shop&&!shopRecordIsActive_(record)){if(token)CacheService.getScriptCache().remove(cleanText_(token));throw new Error('This shop link has been deleted or disabled. Ask the Master Owner for a new link.');}return record;}
function activeShopCacheKey_(){const key=cleanText_(Session.getTemporaryActiveUserKey&&Session.getTemporaryActiveUserKey());return POS_ACTIVE_SHOP_CACHE_PREFIX_+(key||'anonymous');}
function setActiveShop_(shopCode){const shop=normaliseShopCode_(shopCode),cache=CacheService.getScriptCache();if(shop)cache.put(activeShopCacheKey_(),shop,21600);else cache.remove(activeShopCacheKey_());}
function activeShopCode_(){return normaliseShopCode_(CacheService.getScriptCache().get(activeShopCacheKey_()));}
function masterFirestoreProjectId_(){return cleanText_(PropertiesService.getScriptProperties().getProperty('FIRESTORE_PROJECT_ID')||FIRESTORE_DEFAULT_PROJECT_ID_);}
function firestoreProjectIdForShop_(shopCode){return masterFirestoreProjectId_();}
function firestoreProjectId_(){const projectId=masterFirestoreProjectId_();if(!projectId)throw new Error('Master shared backend ID is not configured. Open the Master Account and connect the shared backend first.');return projectId;}
/** Every clean installation receives its own private data area. */
function firestoreStoreId_() {
  const properties = PropertiesService.getScriptProperties();
  let id = cleanText_(properties.getProperty('POS_STORE_ID'));
  if (!id) {
    id = 'store-' + ScriptApp.getScriptId().replace(/[^a-zA-Z0-9_-]/g, '').slice(-32);
    properties.setProperty('POS_STORE_ID', id);
  }
  return id;
}
function firestoreDocId_(name) { return encodeURIComponent(String(name).replace(/\//g, '_')); }
function firestoreSheetPath_(name) { return FIRESTORE_ROOT_COLLECTION_ + '/' + firestoreDocId_(name); }
function firestoreRowPath_(name, row) { return firestoreSheetPath_(name) + '/rows/r' + String(row).padStart(12, '0'); }
function firestoreUrl_(path) { return 'https://firestore.googleapis.com/v1/projects/' + encodeURIComponent(firestoreProjectId_()) + '/databases/(default)/documents/' + path; }
function firestoreRequest_(url, options) {
  const opt = options || {}; opt.headers = Object.assign({}, opt.headers || {}, { Authorization: 'Bearer ' + ScriptApp.getOAuthToken() }); opt.muteHttpExceptions = true;
  const response = UrlFetchApp.fetch(url, opt), code = response.getResponseCode(), body = response.getContentText();
  if (code >= 200 && code < 300) return body ? JSON.parse(body) : {};
  if (code === 404) return null;
  if (code === 403) {
    throw new Error('Permission required for the shared backend project: ' + firestoreProjectId_() + '\n\nOpen this project in Google Cloud / Firebase, then on the IAM page do these steps:\n1. In New principals, add the Google account or service account that runs this web app.\n2. In Select a role, choose Cloud Datastore User or Firestore User.\n3. Click Save.\n4. Come back to the app and press Retry / Connect and activate.\n\nFirestore / Firebase API may also need to be enabled for this shared project.\n\nNote: Google security rules mean the software cannot auto-allow IAM. The owner/customer must click Save on Google\'s IAM page.');
  }
  throw new Error('Firestore request failed (' + code + '): ' + (body || 'Unknown error'));
}
function firestoreDocumentExists_(path) { return !!firestoreReadDocument_(path); }
function firestoreReadDocument_(path) { const data = firestoreRequest_(firestoreUrl_(path), { method: 'get' }); return data ? firestoreFieldsToObject_(data.fields || {}) : null; }
function firestoreWriteDocument_(path, object, createOnly) {
  let url = firestoreUrl_(path); if (createOnly) url += '?currentDocument.exists=false';
  return firestoreRequest_(url, { method: 'patch', contentType: 'application/json', payload: JSON.stringify({ fields: firestoreObjectToFields_(object) }) });
}
function firestoreDeleteDocument_(path) { return firestoreRequest_(firestoreUrl_(path), { method:'delete' }); }
/** Writes many Firestore documents in a few requests. Used for backup restore so large stock lists do not time out. */
function fir9yMnTm4NSzvG9rrwjM2ec8xZgh1cafXH8_(documents) {
  const project = firestoreProjectId_(), base = 'projects/' + project + '/databases/(default)/documents/';
  for (let start = 0; start < documents.length; start += 400) {
    const batch = documents.slice(start, start + 400).map(function (document) {
      return { update:{ name:base + document.path, fields:firestoreObjectToFields_(document.object) } };
    });
    const url = 'https://firestore.googleapis.com/v1/projects/' + encodeURIComponent(project) + '/databases/(default)/documents:batchWrite';
    firestoreRequest_(url, { method:'post', contentType:'application/json', payload:JSON.stringify({ writes:batch }) });
  }
}
function firestoreListRows_(sheetName) {
  let token = '', output = [];
  do { const url = firestoreUrl_(firestoreSheetPath_(sheetName) + '/rows') + '?pageSize=1000' + (token ? '&pageToken=' + encodeURIComponent(token) : ''); const data = firestoreRequest_(url, { method: 'get' }) || {}; (data.documents || []).forEach(function (d) { output.push(firestoreFieldsToObject_(d.fields || {})); }); token = data.nextPageToken || ''; } while (token);
  return output.sort(function (a, b) { return Number(a.row || 0) - Number(b.row || 0); });
}
function firestoreSafeValue_(value) { return value instanceof Date ? value.toISOString() : value; }
function firestoreObjectToFields_(object) { const out = {}; Object.keys(object || {}).forEach(function (key) { out[key] = firestoreValue_(object[key]); }); return out; }
function firestoreValue_(value) {
  ...
  if (typeof value === 'object') return { mapValue: { fields: firestoreObjectToFields_(value) } };
  return { stringValue: String(value) };
}
function firestoreFieldsToObject_(fields) { const out = {}; Object.keys(fields || {}).forEach(function (key) { out[key] = firestoreValueToJs_(fields[key]); }); return out; }
function firestoreValueToJs_(value) {
  ...
  if ('arrayValue' in value) return (value.arrayValue.values || []).map(firestoreValueToJs_); if ('mapValue' in value) return firestoreFieldsToObject_(value.mapValue.fields || {}); return '';
}

/** Run once from the Apps Script editor to use another Firebase project ID, if needed. */
function configureFirestore(projectId) { projectId = cleanText_(projectId); if (!projectId) throw new Error('Enter the Firebase project ID.'); PropertiesService.getScriptProperties().setProperty('FIRESTORE_PROJECT_ID', projectId); return 'Firestore configured: ' + projectId; }

/**
 * Optional one-time import for existing billing data. It only reads the old
 * spreadsheet during this manual import; normal app operation never uses it.
 * Run this once from the Apps Script editor with the old spreadsheet ID.
 */
function migrateLegacySpreadsheetToFirestore(legacySpreadsheetId) {
  legacySpreadsheetId = cleanText_(legacySpreadsheetId);
  if (!legacySpreadsheetId) throw new Error('Enter the old Google Spreadsheet ID.');
  return withLock_(function () {
    const source = SpreadsheetApp.openById(legacySpreadsheetId), target = getSpreadsheet_(), result = [];
    Object.keys(APP.SHEETS).forEach(function (key) {
      const name = APP.SHEETS[key], legacySheet = source.getSheetByName(name);
      if (!legacySheet || legacySheet.getLastRow() === 0) return;
      const destination = getOrCreateSheet_(target, name);
      if (destination.getLastRow() > 1) throw new Error('Firestore already has data in "' + name + '". Import was stopped to prevent duplicates.');
      const values = legacySheet.getDataRange().getValues();
      if (values.length) destination.getRange(1, 1, 1, values[0].length).setValues([values[0]]);
      values.slice(1).forEach(function (row) { destination.appendRow(row); });
      result.push(name + ': ' + Math.max(0, values.length - 1) + ' rows');
    });
    return result.length ? 'Imported ' + result.join(', ') : 'No legacy data was found.';
  });
}

/** One-time import button for the original Shan Computer Systems Billing spreadsheet. */
function importShanBillingDataToFirestore() {
  return migrateLegacySpreadsheetToFirestore('1W1S6aBDUOMpBxuDY9-17DhDdm4QQubpaULTqOfWxr-U');
}
...
function getReport(request) {
  ...
  else { const d = date ? new Date(date + 'T00:00:00') : new Date(); if (isNaN(d)) throw new Error('Choose a valid date.'); start = new Date(d.getFullYear(), d.getMonth(), d.getDate()); end = new Date(d.getFullYear(), d.getMonth(), d.getDate() + 1); title = Utilities.formatDate(start, Session.getScriptTimeZone(), 'yyyy-MM-dd'); }
...
function restoreSimpleInvoiceStockTarget_(parsed, target) {
    ...
    const meta = firestoreReadDocument_(firestoreSheetPath_(target)) || { headers:[], nextRow:2 };
    ...
    products.forEach(function (record, index) {
      ...
      if (rowNumber) {
        ...
        writes.push({ path:firestoreRowPath_(target, rowNumber), object:{ row:rowNumber, values:values.map(firestoreSafeValue_) } });
        updated++;
      } else {
        const newRow = nextRow++;
        writes.push({ path:firestoreRowPath_(target, newRow), object:{ row:newRow, values:[id, name, cost, price, qty, trackStock, 'YES', category, '', '', barcode].map(firestoreSafeValue_) } });
        existing[key] = newRow;
        added++;
      }
      restoredIds.push(id);
    ...
    writes.push({ path:firestoreSheetPath_(target), object:meta });
    fir9yMnTm4NSzvG9rrwjM2ec8xZgh1cafXH8_(writes);
...
function removeSimpleInvoiceBackupProducts(request) {
  ...
  return withLock_(function () {
    ...
    indexRows.slice(1).forEach(function (row, index) {
      ...
      indexWrites.push({ path:firestoreRowPath_(LEGACY_BACKUP_RESTORE_INDEX_SHEET_, rowNumber), object:{ row:rowNumber, values:values.map(firestoreSafeValue_) } });
    });
    ...
    targets.forEach(function (target) {
      ...
      rows.slice(1).forEach(function (row, index) {
        ...
        writes.push({ path:firestoreRowPath_(target, rowNumber), object:{ row:rowNumber, values:values.map(firestoreSafeValue_) } }); removed++;
      });
      if (writes.length) fir9yMnTm4NSzvG9rrwjM2ec8xZgh1cafXH8_(writes);
    });
    ...
    if (imports && imports.getLastRow() > 1) {
      ...
      rows.slice(1).forEach(function (row, index) {
        ...
        const values = row.slice(); values[4] = 'REMOVED'; const rowNumber = index + 2;
        writes.push({ path:firestoreRowPath_(LEGACY_BACKUP_IMPORTS_SHEET_, rowNumber), object:{ row:rowNumber, values:values.map(firestoreSafeValue_) } });
      });
      if (writes.length) fir9yMnTm4NSzvG9rrwjM2ec8xZgh1cafXH8_(writes);
    }
...
function legacyBackupParse_(dataUrl) {
  ...
  if (bytes.length > 15 * 1024 * 1024) throw new Error('Backup is too large. Please use a file under 15 MB.');
  let files;
  try { files = Utilities.unzip(Utilities.newBlob(bytes, 'application/zip', 'sinvoice.bak')); }
  ...
  const fingerprint = Utilities.base64Encode(digest).replace(/[+/=]/g, '').slice(0, 24);
...
function appendSerialStock_(ss,batch){
  ...
  batch.serials.forEach(function(entry){
    ...
    sh.appendRow(['SER-'+Utilities.getUuid().slice(0,8).toUpperCase(),new Date(),batch.purchaseNo,batch.itemId,batch.itemName,serial,batch.warranty,'AVAILABLE','','',imei]);
  });
}
...
function submitPaymentSetupRequest(request) {
  ...
  return withLock_(function () {
    ...
    const id = 'PSR-' + Utilities.getUuid().slice(0, 8).toUpperCase();
